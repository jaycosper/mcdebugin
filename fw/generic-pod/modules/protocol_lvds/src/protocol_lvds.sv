...
module protocol_lvds #(
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // payload from regs_in.status
            {tx_buf[5],tx_buf[6],tx_buf[7],tx_buf[8]}  = regs_in.status.status;
            {tx_buf[9],tx_buf[10],tx_buf[11],tx_buf[12]} = regs_in.status.hash;
            tx_buf[13] = 8'hBC; // EOF
